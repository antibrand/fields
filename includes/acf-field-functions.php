...
function acf_flush_field_cache( $field ) {
	...
	wp_cache_delete( acf_cache_key("acf_get_field_post:name:{$field['name']}"), 'fields' );
	wp_cache_delete( acf_cache_key("acf_get_field_post:key:{$field['key']}"), 'fields' );
	
	// Flush cached array of post_ids for this field's parent.
	wp_cache_delete( acf_cache_key("acf_get_field_posts:{$field['parent']}"), 'fields' );
}
 ...
 * @return	void
 */
function acf_prefix_fields( &$fields, $prefix = 'fields' ) {
	
	// Loopover fields.
	foreach( $fields as &$field ) {
		
		// Replace 'fields' with $prefix.
